...
Shader::Shader(std::string vertexSPath, std::string pixelSPath)
{
...
void Shader::CreateVertexShader(std::string path)
{
    ...
    if (FAILED(hr))
    {
        LOG_CORE_ERROR("Failed creating input layout: %s", path.c_str());
    }
...
{ 
    ...
    if(hasInputArray) GraphicsDevice::Get().context->CSSetShaderResources(0, 1, &mInputView);
    ...
    if (SUCCEEDED(hr))
    {
        //int* dataView = reinterpret_cast<int*>(mappedResource.pData);
        outputData = reinterpret_cast<int*>(mappedResource.pData);

        std::ofstream MyFile("result.txt");
        for (size_t i = 0; i < 64 * 64; i++)
        {
            MyFile << ((int*)outputData)[i] << " ";
        }
...
void ComputeShader::LoadInput(uint32_t numInputElements, uint32_t inputElementSize)
{
    ...
    */
    ...
    hr = GraphicsDevice::Get().device->CreateBuffer(&constantDataDesc, nullptr, &mInputBuffer);
    assert(SUCCEEDED(hr));
