...
Texture::Texture(std::string name, bool linearFilter, bool flipTex)
{
    int nrChannels;
    stbi_set_flip_vertically_on_load(flipTex);
    unsigned char* data = stbi_load(name.c_str(), &m_Width, &m_Height, &nrChannels, (std::filesystem::path(name).extension().string() == ".png") ? 0 : 4);
    if (data == nullptr)
    {
